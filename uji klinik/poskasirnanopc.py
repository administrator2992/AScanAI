...
tk.Label(form_tax, text="Tax (%) :", font=("Segoe UI", 12), bg="white").grid(row=0, column=0, sticky="w", padx=10,
                                                                             pady=10)
...
def load_inventaris():
    ...
    def show_kategori_items(nama_kategori):
        ...
        headers = ["No", "Gambar", "Nama", "Harga", "Stok", "Discount", "Kategori", "Action"]
        widths = [6, 20, 40, 20, 10, 15, 25, 15]  # total = 145 "grid units", proporsional full HD
        ...
        for item_id, item in item_data.items():
            ...
            tk.Label(scrollable_frame, text=row_num, bg="white", relief="ridge", bd=1, anchor="center").grid(
                row=row_num, column=0, sticky="nsew", padx=1, pady=1)

            img_item = tk.Label(scrollable_frame, text="Loading...", bg="white", relief="ridge", bd=1)
            img_item.grid(row=row_num, column=1, sticky="nsew", padx=1, pady=1)
            if gambar:
            ...
            tk.Label(scrollable_frame, text=f"Rp {harga:,}", bg="white", anchor="center", relief="ridge", bd=1).grid(
                row=row_num, column=3, sticky="nsew", padx=1, pady=1)

            # === Stok ===
            frame_qty = tk.Frame(scrollable_frame, bg="white", relief="ridge", bd=1)
            frame_qty.grid(row=row_num, column=4, sticky="nsew", padx=1, pady=1)
            ...
            frame_discount.grid(row=row_num, column=5, sticky="nsew", padx=1, pady=1)
            ...
            tk.Label(scrollable_frame, text=nama_kategori, bg="white", anchor="center", relief="ridge", bd=1).grid(
                row=row_num, column=6, sticky="nsew", padx=1, pady=1)

            # === Action ===
            frame_action = tk.Frame(scrollable_frame, bg="white", relief="ridge", bd=1)
            frame_action.grid(row=row_num, column=7, sticky="nsew", padx=1, pady=1)
...
tk.Label(checkout_left, text="🧺 Keranjang Belanja", font=("Segoe UI", 14, "bold"), bg="white").pack(anchor="w",
                                                                                                    pady=(5, 2))
...
scroll_keranjang_frame.bind("<Configure>",
                            lambda e: canvas_keranjang.configure(scrollregion=canvas_keranjang.bbox("all")))
...
metode_cb = ttk.Combobox(checkout_left, textvariable=metode_var, values=["Tunai", "QRIS", "Debit"], state="readonly",
                         width=32)
metode_cb.pack(pady=(0, 5))

ttk.Button(form_area, text="+ Tambah ke Keranjang", command=lambda: tambah_keranjang()).grid(row=2, column=0,
...
def add_card_keranjang(nama, jumlah, harga, gambar_url):
    ...
    tk.Button(control, text="❌", width=2, command=lambda: [card.destroy(), hapus_item(nama)], bg="#e74c3c",
...
def tampilkan_struk_di_payment_frame(trx_id, total, metode, items):
    ...
    tk.Label(wrapper, text="✅ Pembayaran Berhasil", font=("Segoe UI", 14, "bold"), fg="green", bg="white").pack(
        pady=(0, 10))
...
def simpan_transaksi_multi():
    ...
    bayar_entry = ttk.Entry(inner, textvariable=bayar_var, font=("Segoe UI", 14), justify="right", state="readonly",
                            width=20)
...
# Scrollable area
# Bungkus dalam container tengah
canvas_container = tk.Frame(detail_frame4, bg="white")
canvas_container.pack(anchor="center", pady=20)

# Canvas dan scrollbar di dalam container
canvas4 = tk.Canvas(canvas_container, bg="white", highlightthickness=0, width=380, height=1080)
scrollbar4 = tk.Scrollbar(canvas_container, orient="vertical", command=canvas4.yview)

scrollable_frame4 = tk.Frame(canvas4, bg="white")


def update_scrollbar_visibility():
    canvas4.update_idletasks()  # pastikan ukuran terbaru
    bbox = canvas4.bbox("all")
    if bbox and bbox[3] > canvas4.winfo_height():  # bbox[3] = bottom
        scrollbar4.pack(side="right", fill="y")
    else:
        scrollbar4.pack_forget()


scrollable_frame4.bind(
    "<Configure>",
    lambda e: [canvas4.configure(scrollregion=canvas4.bbox("all")), update_scrollbar_visibility()]
)
...
for i, (k, v) in enumerate(labels_info.items()):
    tk.Label(info_frame, text=k + ":", font=("Segoe UI", 11, "bold"), bg="white").grid(row=i, column=0, sticky="w",
                                                                                       pady=2)
...
for col, head in enumerate(headers):
    tk.Label(table_frame, text=head, font=("Segoe UI", 10, "bold"), bg="#f1f1f1", padx=10, pady=6, relief="ridge").grid(
...
def tampilkan_detail_di_frame4(trx_id4):
    ...
    for i, item4 in enumerate(barang_list4, start=1):
        ...
        tk.Label(table_frame, text=nama, bg="white", font=("Segoe UI", 10),
                 padx=10, pady=5, anchor="center", justify="center").grid(row=i, column=0, sticky="nsew")

        tk.Label(table_frame, text=jumlah, bg="white", font=("Segoe UI", 10),
                 anchor="center", justify="center").grid(row=i, column=1, sticky="nsew")

        tk.Label(table_frame, text=f"Rp {harga:,}", bg="white", font=("Segoe UI", 10),
                 anchor="center", justify="center").grid(row=i, column=2, sticky="nsew")

        tk.Label(table_frame, text=f"Rp {total:,}", bg="white", font=("Segoe UI", 10),
                 anchor="center", justify="center").grid(row=i, column=3, sticky="nsew")
...
def load_riwayat():
    ...
    widths = [20, 25, 30, 25, 25, 20]  # total 155 grid width
    ...
    for col, (text, w) in enumerate(zip(headers, widths)):
        ...
        tk.Label(
            ...
            padx=8,
            pady=6
        ).grid(row=0, column=col, sticky="nsew", padx=4)  # ← Tambahkan padx=4
    ...
    for trx_id, val in sorted_data:
        ...
        for col, val in enumerate(cells):
            tk.Label(scroll_frame, text=val, bg=BG_COLOR, font=FONT, padx=6, pady=4).grid(row=row_idx, column=col,
                                                                                          sticky="nsew", padx=4)

        # Kolom Aksi
        aksi_frame = tk.Frame(scroll_frame, bg=BG_COLOR)
        aksi_frame.grid(row=row_idx, column=5, sticky="nsew", padx=4)

        btn_edit = tk.Button(aksi_frame, text="✏️", font=FONT, bd=0, bg=BG_COLOR,
                             command=lambda tid=trx_id: tampilkan_detail_di_frame4(tid))
        btn_hapus = tk.Button(aksi_frame, text="🗑️", font=FONT, bd=0, fg="red", bg=BG_COLOR,
                              command=lambda tid=trx_id: konfirmasi_hapus_transaksi(tid))

        # Buat frame isi tombol biar center
        tombol_frame = tk.Frame(aksi_frame, bg=BG_COLOR)
        tombol_frame.pack(expand=True)  # ini bikin center otomatis

        btn_edit.pack(side="left", padx=20)
        btn_hapus.pack(side="left", padx=20)
...
kategori_inner_frame2.bind("<Configure>",
                           lambda e: kategori_canvas2.configure(scrollregion=kategori_canvas2.bbox("all")))
...
# list barang isi kategori
frame_daftar_barang = tk.Frame(tambah, bg="white")
frame_daftar_barang.pack_forget()  # default disembunyikan

barang_images = []  # simpan referensi gambar agar tidak hilang


def tampilkan_daftar_barang(kategori):
    # Bersihkan frame dulu
    for widget in frame_daftar_barang.winfo_children():
        widget.destroy()
    barang_images.clear()

    frame_daftar_barang.pack(pady=10)
    form_tambah.pack_forget()
    form_kategori_baru.pack_forget()

    try:
        daftar = db.reference("inventaris").child(kategori).get() or {}
        item_widgets = []

        for idbrg, data in daftar.items():
            nama = data.get("nama", "Tanpa Nama")
            gambar_url = data.get("gambar", "")

            # Frame item (UI dasar dulu)
            frame_item = tk.Frame(frame_daftar_barang, bg="white", padx=5, pady=5)
            frame_item.pack(side="left", padx=10, pady=10)

            # Frame border khusus untuk gambar
            border_gambar = tk.Frame(frame_item, bg="#DDE6F1", bd=1, relief="solid", padx=2, pady=2)
            border_gambar.pack()

            img_label = tk.Label(border_gambar, text="Loading...", bg="white")
            img_label.pack()

            tk.Label(frame_item, text=nama, bg="white", font=("Segoe UI", 9)).pack(pady=(5, 0))

            item_widgets.append((img_label, gambar_url))  # simpan untuk lazy load nanti

        # Tombol Tambah Barang Baru
        btn_tambah_baru = tk.Frame(frame_daftar_barang, bg="white", cursor="hand2")
        btn_tambah_baru.pack(side="left", padx=10, pady=10)

        canvas_tambah = tk.Canvas(btn_tambah_baru, width=80, height=80, bg="white", highlightthickness=0)
        canvas_tambah.pack()

        canvas_tambah.create_oval(10, 10, 70, 70, fill="#e74c3c", outline="")
        canvas_tambah.create_text(40, 40, text="+", font=("Segoe UI", 28, "bold"), fill="white")

        tk.Label(btn_tambah_baru, text="Add New Product", font=("Segoe UI", 10, "bold"), bg="white", fg="#556677").pack(
            pady=(5, 0))

        def klik_add():
            form_tambah.pack()
            frame_daftar_barang.pack_forget()

        btn_tambah_baru.bind("<Button-1>", lambda e: klik_add())
        canvas_tambah.bind("<Button-1>", lambda e: klik_add())

        # Mulai thread lazy load gambar
        def load_images_async():
            for img_label, url in item_widgets:
                if url:
                    try:
                        response = requests.get(url, stream=True, timeout=5)
                        img = Image.open(response.raw).resize((80, 80))
                        img_tk = ImageTk.PhotoImage(img)
                        barang_images.append(img_tk)  # supaya tidak garbage collected
                        img_label.config(image=img_tk, text="")
                        img_label.image = img_tk
                    except Exception as e:
                        img_label.config(text="❌", image="")

        threading.Thread(target=load_images_async, daemon=True).start()

    except Exception as e:
        messagebox.showerror("Error", f"Gagal menampilkan barang: {e}")
...
def tampilkan_kategori_kotak2():
    ...
    for idx, nama_kategori in enumerate(daftar_kategori):
        ...
        def safe_update_widget(widget, **kwargs):
            """Safely update a Tkinter widget from any thread"""
            try:
                if widget and widget.winfo_exists():
                    widget.after(0, lambda w=widget, k=kwargs: w.config(**k) if w.winfo_exists() else None)
            except Exception as e:
        ...
        def load_detail_kategori(nama=nama_kategori, label_img=img_label, label_nama=label_nama, idx=idx):
            try:
                ...
                # Update label name safely
                safe_update_widget(label_nama, text=nama_tampil)
                ...
                if gambar_url:
                    try:
                        ...
                        if not hasattr(load_detail_kategori, 'image_refs'):
                            load_detail_kategori.image_refs = []
                        load_detail_kategori.image_refs.append(img_tk)
                        
                        # Update image safely
                        def update_image():
                            if label_img.winfo_exists():
                                label_img.config(image=img_tk, text="")
                                label_img.image = img_tk
                        
                        label_img.after(0, update_image)
                    except Exception as img_error:
                        print(f"[Image Load Error] {nama} | {img_error}")
                        safe_update_widget(label_img, text="❌", font=("Arial", 14), image="", compound="top")
                else:
                    safe_update_widget(label_img, text="[No Image]", image="")
...
tk.Label(form_kategori_baru, text="Nama Kategori Baru:", background="white").grid(row=0, column=0, sticky="e", padx=5,
                                                                                  pady=5)
...
tk.Label(form_kategori_baru, text="Gambar Kategori:", background="white").grid(row=1, column=0, sticky="e", padx=5,
                                                                               pady=5)
...
MODEL_PATH = "best_nasi_rknn_model"
USE_CAMERA = True
...
def update_keranjang_final():
    # 1. Get current detections and check for Nasi Campur combinations first
    global nama_terdeteksi
    
    # 2. Update nama_terdeteksi with current detections
    nama_terdeteksi = set(last_detected_items.keys())
    
    # 3. Check for Nasi Campur combinations using the actual detected items
    nasi_campur_results = get_nasi_campur_price(last_detected_items.keys()) if last_detected_items else None
    
    # 4. Remove AI items that are no longer detected or part of combos
    items_to_remove = []
    for nama in list(keranjang_final.keys()):
        if keranjang_final[nama].get("source") == "ai":
            # Check if this is an individual item that's now part of a combo
            if nasi_campur_results and nama in ['Nasi', 'Orek', 'Terong', 'Kentang']:
                items_to_remove.append(nama)
            # Check if this item is no longer detected
            elif nama not in last_detected_items:
                items_to_remove.append(nama)
    
    # Remove the items
    for item in items_to_remove:
        if item in keranjang_final:
            del keranjang_final[item]
    
    # 5. Add/update Nasi Campur combinations if any
    if nasi_campur_results:
        # Add all Nasi Campur Rev combos to the cart
        for result in nasi_campur_results:
            if not result or result[0] <= 0:  # Skip if no result or price <= 0
                continue
                
            price, discount, display_name = result
            
            # Add the combo to the cart
            keranjang_final[display_name] = {
                "quantity": 1,
                "harga": price,
                "total": price,
                "discount": discount,
    ...
    # 6. Check if we have any combos in the cart
    has_combo = any(item.get('is_combo', False) for item in keranjang_final.values())
    
    # 7. Initialize hidden_items if it doesn't exist
    if not hasattr(update_keranjang_final, 'hidden_items'):
        update_keranjang_final.hidden_items = {}
    
    # 8. If we have a combo, hide all non-combo items
    if has_combo:
        # Store all current non-combo items in hidden_items
        for item_name in list(keranjang_final.keys()):
            if not keranjang_final[item_name].get('is_combo'):
                update_keranjang_final.hidden_items[item_name] = keranjang_final[item_name]
        
        # Clear the cart and only keep combo items
        keranjang_final.clear()
        for result in nasi_campur_results:
            if result and result[0] > 0:  # Only add if price > 0
                price, discount, display_name = result
                keranjang_final[display_name] = {
                    "quantity": 1,
                    "harga": price,
        ...
        # No combo, restore hidden items
        for item_name, item_data in update_keranjang_final.hidden_items.items():
            if item_name not in keranjang_final:
                keranjang_final[item_name] = item_data
        update_keranjang_final.hidden_items = {}
    
    # 9. Process all detected items if no combo
    if not has_combo:
        for nama, data in last_detected_items.items():
            ...
            # Add/update the item if it's not in cart or needs updating
            if nama not in keranjang_final or keranjang_final[nama].get('source') == 'ai':
                jumlah = data.get("quantity", 1)
                total = harga * jumlah
...
def update_keranjang_dari_AI(detected_now):
    ...
    # Separate Nasi Campur items from others
    nasi_campur_group = {'Nasi', 'Orek', 'Terong', 'Kentang'}
    nasi_campur_detected = [item for item, data in detected_now.items() if item in nasi_campur_group for _ in range(data['quantity'])]
    other_items = {item: data for item, data in detected_now.items() if item not in nasi_campur_group}

    # --- Process Nasi Campur Items --- #
    if nasi_campur_detected:
        # get_nasi_campur_price will return the correct packages and remaining individual items
        nasi_campur_results = get_nasi_campur_price(nasi_campur_detected)
        if nasi_campur_results:
            for price, discount, display_name in nasi_campur_results:
                # Determine if it's a combo or individual item from the name
                is_combo = 'Paket' in display_name or '+' in display_name
                new_detected_items[display_name] = {
                    "quantity": 1, # The function already handles quantity in price/name
                    "harga": price,
                    "total": price,
                    "discount": discount,
                    "is_combo": is_combo,
    ...
    # --- Process Other Non-Nasi Campur Items --- #
    for name, data in other_items.items():
        # This logic remains for items that are not part of Nasi Campur
        price_info = get_inventory_price_discount(data.get('kategori', 'unknown'), name)
        if price_info and price_info[0] > 0:
            price, discount, _ = price_info
            new_detected_items[name] = {
        ...
        # For Nasi Campur items not in any combo, add them individually if price > 0
        if name in nasi_campur_group:
            result = get_inventory_price_discount("Nasi Campur Rev", name)
            if result and result[0] > 0:  # Only add if price > 0
                price, discount, _ = result
                new_detected_items[name] = {
                    "quantity": 1,
                    "harga": price,
                    "total": price,
        ...
        # Add regular items
        elif name not in new_detected_items:
            new_detected_items[name] = data
    
    # Update last_detected_items with the processed items
    last_detected_items.clear()
    last_detected_items.update(new_detected_items)
    
    # Update the cart with the processed detections
...
def get_keranjang_from_deteksi():
    ...
    for name, data in keranjang_final.items():
        # For combo items, ensure we use the correct price and total
        if data.get("is_combo", False):
            result.append({
                "nama": name,
...
def simpan_transaksi_multi2():
    ...
    # Container disesuaikan lebar untuk full HD
    inner = tk.Frame(payment_frame2, bg="white", width=1280)
    inner.pack(padx=40, pady=30, fill="both", expand=True)
    inner.pack_propagate(False)

    left_frame = tk.Frame(inner, bg="white", width=800)
    right_frame = tk.Frame(inner, bg="white", width=460)
    ...
    # ENTRY BAYAR
    bayar_var = tk.StringVar(value="")
    bayar_entry = ttk.Entry(left_frame, textvariable=bayar_var, font=("Segoe UI", 22), justify="right",
                            state="readonly", width=22)
    bayar_entry.pack(pady=(0, 25))
    ...
    for row in pad_buttons:
        ...
        for val in row:
            display = f"{int(val):,}".replace(",", ".") if val.isdigit() and int(val) >= 1000 else val
            btn = tk.Button(
                row_frame,
                text=display,
                font=("Segoe UI", 22, "bold") if val.isdigit() and int(val) >= 1000 else ("Segoe UI", 22),
                width=8,
                height=4,
                command=lambda v=val: update_bayar(v)
            )
    ...
    # --- RIGHT SIDE INFO ---
    def label_kanan(text, var=None, bold=False, pad=(10, 10)):
        tk.Label(right_frame, text=text, font=("Segoe UI", 22, "bold" if bold else "normal"),
                 bg="white", fg="#333").pack(anchor="w", padx=10, pady=(pad[0], 2))
        label = tk.Label(right_frame, textvariable=var, font=("Segoe UI", 22), bg="white") if var else \
            tk.Label(right_frame, text="", font=("Segoe UI", 22), bg="white")
        label.pack(anchor="w", padx=10, pady=(0, pad[1]))
        return label

    label_kanan("Total", pad=(0, 10))
    tk.Label(right_frame, text=f"idr {final_price:,}".replace(",", "."), font=("Segoe UI", 22, "bold"),
             bg="white").pack(anchor="w", padx=10)

    label_kanan("Paid", var=bayar_var, pad=(20, 10))

    tk.Frame(right_frame, height=2, bg="#ccc").pack(fill="x", pady=20)

    tk.Label(right_frame, text="Change", font=("Segoe UI", 22, "bold"), bg="white").pack(anchor="w", padx=10)
    kembalian_label = tk.Label(right_frame, text="", font=("Segoe UI", 22, "bold"), background="white",
                               foreground="green")
    kembalian_label.pack(anchor="w", padx=10, pady=10)
...
CLASS_NAMES = ["Tumis Ebi", "Kacang", "Kentang", "Nasi", "Orek", "Terong"]

# Initialize prices and discounts
PRICES = {name: 0 for name in CLASS_NAMES}  # Initialize all prices to 0
DISCOUNTS = {name: 0 for name in CLASS_NAMES}  # Initialize all discounts to 0

# Define box colors for each class
BOX_COLORS = {
    "Tumis Ebi": "#F51B1B",
    "Kacang": "#EBA74F",
    "Kentang": "#B6C912",
...
def get_nasi_campur_price(detected_items):
    """
    Determines the price for Nasi Campur combinations based on detected items.
    Prioritizes Paket B, then Paket A, then individual items.
    """
    results = []
    ...
    # --- Paket B Detection (Nasi + Orek + Terong + Kentang) --- #
    if (
        item_counts.get('Nasi', 0) > 0
        and item_counts.get('Orek', 0) > 0
        and item_counts.get('Terong', 0) > 0
        and item_counts.get('Kentang', 0) > 0
    ):
        result = get_inventory_price_discount("Nasi Campur Rev", "Paket B")
        if result and result[0] > 0:
            price, discount, _ = result
            results.append((price, discount, 'Paket B'))
            # Decrement counts for used items
            item_counts['Nasi'] -= 1
            item_counts['Orek'] -= 1
            item_counts['Terong'] -= 1
            item_counts['Kentang'] -= 1

    # --- Paket A Detection (Nasi + 2 side dishes) --- #
    paket_a_combinations = [
        (['Orek', 'Terong'], 'Paket A'),
        (['Orek', 'Kentang'], 'Paket A'),
        (['Terong', 'Kentang'], 'Paket A')
    ]

    # Loop to create as many Paket A as possible
    while True:
        paket_a_found = False
        if item_counts.get('Nasi', 0) > 0:
            for items, display_name in paket_a_combinations:
                if all(item_counts.get(it, 0) > 0 for it in items):
                    result = get_inventory_price_discount("Nasi Campur Rev", "Paket A")
                    if result and result[0] > 0:
                        price, discount, _ = result
                        results.append((price, discount, display_name))
                        # Decrement counts for used items
                        item_counts['Nasi'] -= 1
                        for it in items:
                            item_counts[it] -= 1
                        paket_a_found = True
                        break # Restart the loop to check for more Paket A
        if not paket_a_found:
            break # Exit if no more Paket A can be formed

    # --- Process Remaining Individual Items --- #
    for item, count in item_counts.items():
        if count > 0 and item != 'Nasi':  # Nasi is never sold individually
            price_info = get_inventory_price_discount("Nasi Campur Rev", item)
            if price_info and price_info[0] > 0:
                display_name = f"{item} (x{count})" if count > 1 else item
                total_price = price_info[0] * count
                results.append((total_price, price_info[1], display_name))

    return results if results else None

def get_inventory_price_discount(kategori, nama_item):
    """Get price and discount for an item from inventory."""
    try:
        # First try to find in the in-memory data_inventaris
        if kategori in data_inventaris:
            for item_id, item_data in data_inventaris[kategori].items():
                if isinstance(item_data, dict) and item_data.get("nama") == nama_item:
                    return (
                        item_data.get("harga", 0),
                        item_data.get("discount", 0),
                        f"{kategori} - {nama_item}" if kategori != "Nasi Campur Rev" else nama_item
                    )
        
        # If not found in memory, try to fetch from database (without relying on order_by_child)
        try:
            kategori_ref = db.reference(f"inventaris/{kategori}").get()
            if kategori_ref:
                for item_id, item_data in kategori_ref.items():
                    if isinstance(item_data, dict) and item_data.get("nama") == nama_item:
                        # Update our in-memory cache for future use
                        if kategori not in data_inventaris:
                            data_inventaris[kategori] = {}
                        data_inventaris[kategori][item_id] = item_data
                        return (
                            item_data.get("harga", 0),
                            item_data.get("discount", 0),
                            f"{kategori} - {nama_item}" if kategori != "Nasi Campur Rev" else nama_item
    ...
    print(f"Warning: Could not find pricing for {kategori}/{nama_item}")
    return (0, 0, nama_item)  # Return the item name even if price not found

# Update prices and discounts from inventory if available
for kategori, kategori_data in data_inventaris.items():
    for item_id, item in kategori_data.items():
        nama = item.get("nama")
        if nama in CLASS_NAMES:  # Only process if the item is in our defined classes
            harga = item.get("harga")
            diskon = item.get("discount", 0)
            if harga is not None:
                PRICES[nama] = harga
            if diskon is not None:
                DISCOUNTS[nama] = diskon
...
# model.to(device)
device = 0 if torch.cuda.is_available() else "cpu/npu"
print(device)
print(torch.version.cuda)  # Harusnya bukan None
print(torch.backends.cudnn.enabled)  # Harusnya True
...
# === GUI Setup ===
canvas = Canvas(home2, width=1920, height=1080, bg="white", bd=0, highlightthickness=0)
...
payment_icons = [PhotoImage(file=relative_to_assets(fn)) for fn in
                 ["image_4.png", "image_5.png", "image_6.png", "image_7.png", "image_8.png"]]
...
canvas.create_rectangle(577 + 200, 10, 1024 + 200, 708, fill="white", outline="")  # geser kanan
canvas.create_image(769 + 200, 70, image=ci)  # geser kanan
canvas.create_text(800 + 200, 55, anchor="nw", text="Cart", fill="#DBA576",
                   font=("FamiljenGrotesk Bold", -24))  # geser kanan
...
transaksi_label.place(x=775, y=690)
...
for i, tx in enumerate(tx_types):
    ...
    btn.place(x=x_start + i * 110, y=690, width=100, height=30)
    tx_buttons[tx] = btn
...
current_order_id = 1
order_id = canvas.create_text(845, 135, anchor="nw", text=f"Order #[{current_order_id:04d}] --:--:--", fill="#000000",
...
def tampilkan_kategori(parent_frame, y=500):
    ...
    spacing = 20
    height_image = 80
    max_canvas_width = 700  # BATAS LEBAR MAKSIMAL
...
img_id = canvas.create_image(400, 250, image=icon_cam)
count_id = canvas.create_text(460, 500, anchor="nw", text="Total Items: [0] Items", fill="#745540", font=("Arial", -20))

footer_y = 730  # Posisi awal

# Baris 1: Total sebelum tax
canvas.create_text(790, footer_y, anchor="nw", text="Subtotal", fill="#000000", font=("microsoft sans serif", -22))
subtotal_id = canvas.create_text(1010, footer_y, anchor="ne", text="IDR 0", fill="#000000",
                                 font=("microsoft sans serif", -24))

# Baris 2: Pajak
canvas.create_text(790, footer_y + 30, anchor="nw", text="Tax", fill="#000000", font=("microsoft sans serif", -22))
tax_amount_id = canvas.create_text(1010, footer_y + 30, anchor="ne", text="IDR 0", fill="#000000",
                                   font=("microsoft sans serif", -24))

# Baris 3: Diskon
canvas.create_text(790, footer_y + 60, anchor="nw", text="Discount", fill="#000000", font=("microsoft sans serif", -22))
discount_amount_id = canvas.create_text(1010, footer_y + 60, anchor="ne", text="IDR 0", fill="#000000",
                                        font=("microsoft sans serif", -24))

# Baris 4: Total akhir
canvas.create_text(790, footer_y + 90, anchor="nw", text="Total", fill="#000000",
                   font=("microsoft sans serif", -22, "bold"))
price_id = canvas.create_text(1010, footer_y + 90, anchor="ne", text="IDR 0", fill="#000000",
...
).place(x=800, y=payment_y_start + 5)

metode_var = tk.StringVar(value="")  # default kosong

payment_methods = [
    ("Cash", 920, payment_y_start),
    ("Card", 1010, payment_y_start),
    ("QR", 1100, payment_y_start),
]
...
continue_btn.place(x=920, y=payment_y_start + 35)
...
def tta_detect(frame):
    ...
    for s in scales:
        for f in flips:
            img = cv2.resize(frame, None, fx=s, fy=s) if s != 1.0 else frame.copy()
            if f: img = cv2.flip(img, 1)
            ...
            for m, b, cl, cf in zip(masks, boxes, classes, confs):
                mask = cv2.resize(m.astype(np.uint8), (w, h), interpolation=cv2.INTER_NEAREST)
                if f: mask = cv2.flip(mask, 1)
                preds.append((mask, b, cl, cf))
    if not preds:
        return frame.copy(), [], [], [], []
    rects, scores = [], []
...
def go_next_page():
    ...
    if current_page < max_page:
        # Hapus tombol halaman sebelumnya
        for btn_minus, btn_plus in qty_buttons.values():
            btn_minus.destroy()
            btn_plus.destroy()
        qty_buttons.clear()
...
def update_frame():
    ...
    for mask, box, cls_idx, conf in zip(masks, boxes, classes, confs):
        ...
        if mask.shape[0] != disp.shape[0] or mask.shape[1] != disp.shape[1]:
            mask = cv2.resize(mask.astype(np.uint8), (disp.shape[1], disp.shape[0]), 
                            interpolation=cv2.INTER_NEAREST)
        
        # Convert mask to binary and ensure it's 8-bit single channel
        _, binary_mask = cv2.threshold(mask.astype(np.uint8), 0.5, 255, cv2.THRESH_BINARY)
        ...
        # Create a colored overlay with the same dimensions as the display
        overlay = np.zeros_like(disp)
        overlay[mask_cleaned > 0] = bgr
        
        # Create a mask for the object area with smooth edges
        mask_float = mask_cleaned.astype(np.float32) / 255.0
        mask_blurred = cv2.GaussianBlur(mask_float, (11, 11), 0)
        
        # Create a 3-channel mask for blending
        mask_3ch = cv2.merge([mask_blurred] * 3)
        
        # Apply the overlay with transparency only to the object area
        alpha = 0.5  # Slightly more visible overlay
        disp = (disp * (1 - mask_3ch) + overlay * mask_3ch * alpha + disp * (1 - alpha) * mask_3ch).astype(np.uint8)
        
        # Draw a more precise bounding box
        x1, y1, x2, y2 = box
        cv2.rectangle(disp, (x1, y1), (x2, y2), bgr, 2)
        ...
        label_x2 = min(disp.shape[1], x1 + text_w + 2 * padding)
        label_y2 = y1
        
        # Draw filled rectangle for label background
        cv2.rectangle(disp, (label_x1, label_y1), (label_x2, label_y2), bgr, -1)
        
        # Draw border around label
        cv2.rectangle(disp, (label_x1, label_y1), (label_x2, label_y2), (255, 255, 255), 1)
        
        # Calculate text position (centered in label)
        text_x = label_x1 + (label_x2 - label_x1 - text_w) // 2
        text_y = label_y1 + text_h + padding - 2
        
        # Draw text with shadow for better visibility
        cv2.putText(disp, label, (text_x + 1, text_y + 1), 
    ...
    total_qty = sum(item["quantity"] for item in keranjang_final.values())
    canvas.itemconfigure(count_id, text=f"Total Items: {total_qty} Items")
    ...
    for cls_idx in cnt_map.keys():
        ...
        # Ensure price is a number (float or int)
        harga = float(PRICES[name]) if isinstance(PRICES[name], (int, float, str)) else 0
        total = qty * harga
        detected_now[name] = {
            "quantity": qty,
            "harga": harga,
    ...
    #         widget.destroy()
    for name in list(qty_buttons.keys()):
        if name not in keranjang_final:
            btn_minus, btn_plus = qty_buttons[name]
            btn_minus.destroy()
            btn_plus.destroy()
            del qty_buttons[name]
    ...
    for i, name in enumerate(displayed_items):
        ...
        # Handle case where harga might be a tuple (for Nasi Campur items)
        harga_value = data["harga"]
        if isinstance(harga_value, (list, tuple)):
            unit = float(harga_value[0])  # Take the first value if it's a tuple/list
        else:
            unit = float(harga_value)
            
        # Handle total similarly
        total_value = data["total"]
        if isinstance(total_value, (list, tuple)):
            price = float(total_value[0])
        else:
            price = float(total_value)
            
        discount_percent = float(DISCOUNTS.get(name, 0))
        ...
        # Tombol + dan -
        # Inisialisasi tombol jika belum ada
        if name not in qty_buttons:
            btn_minus = Button(
                home2,
                text="-",
                font=("Arial", 10, "bold"),
                width=2,
                command=partial(update_quantity, name, -1),
                bg="#EEE",
                fg="#333",
                bd=1,
                relief="ridge"
            ...
            btn_minus.is_qty_button = True
            btn_plus.is_qty_button = True
            qty_buttons[name] = (btn_minus, btn_plus)
        else:
            btn_minus, btn_plus = qty_buttons[name]

        # Update posisi tombol setiap frame
        btn_minus.place(x=980, y=y + 10, width=20, height=20)
        btn_plus.place(x=1050, y=y + 10, width=20, height=20)
        btn_minus.lift()
        btn_plus.lift()

        item_ids += [
            canvas.create_text(790, y, anchor="nw", text=name, fill="#000000", font=("microsoft sans serif", -18)),
            canvas.create_text(790, y + 22, anchor="nw", text=format_currency(unit), fill="#000000",
                               font=("microsoft sans serif", -18)),
            canvas.create_image(1025, y + 22, image=Colom_count),
            canvas.create_text(1017.5, y + 10, anchor="nw", text=str(qty), fill="#000000",
                               font=("microsoft sans serif", -20)),
            canvas.create_text(1210, y + 10, anchor="ne", text=format_currency(price), fill="#000000",
                               font=("microsoft sans serif", -18))
        ]

    # Buat tombol satu kali saat awal
    # btn_prev = Button(
    #     home2, text="<<", font=("Arial", 10),
    #     command=go_prev_page,
    #     bg="#DDD", fg="#000", bd=1, relief="ridge"
    # )
    # btn_prev.place(x=800, y=y + 80, width=40, height=25)
    # btn_prev.is_qty_button = True
    # btn_prev.place_forget()  # Sembunyikan dulu
    #
    # btn_next = Button(
    #     home2, text=">>", font=("Arial", 10),
    #     command=go_next_page,
    #     bg="#DDD", fg="#000", bd=1, relief="ridge"
    # )
    # btn_next.place(x=850, y=y + 80, width=40, height=25)
    ...
    for name, data in keranjang_final.items():
        ...
        harga = float(data['harga']) if isinstance(data['harga'], (int, float, str)) else 0.0
        jumlah = int(data['quantity']) if isinstance(data['quantity'], (int, float, str)) else 0
        total = float(harga * jumlah)
        diskon = float(DISCOUNTS.get(name, 0))
        potongan = float(total * (diskon / 100.0))
        keranjang.append({
            'nama': name,
            'jumlah': jumlah,
            'harga': harga,
