...
# tk.Label(inv, text="Inventory List", font=("Segoe UI", 18), bg="white").pack(pady=10)
...
def load_inventaris():
    ...
    def show_kategori_items(nama_kategori):
        ...
        headers = ["Gambar", "Nama", "Harga", "Stok", "Discount", "Kategori", "Edit"]
        ...
        for item_id, item in item_data.items():
            ...
            # === Tombol Edit ===
            btn_edit = tk.Button(scrollable_frame, text="✏️ Edit", bg="white", relief="ridge", bd=1,
                                 command=lambda item_data=item, kategori=nama_kategori: open_edit(item_data, kategori))
            btn_edit.grid(row=row_num, column=6, sticky="nsew", padx=1, pady=1)
...
#
# riw_tree = ttk.Treeview(riw_table_frame, columns=("ID", "Tanggal", "Tipe", "Pembeli", "Metode", "Fitur"), show="headings", height=15)
#
# riw_tree.heading("ID", text="ID Transaksi")
# riw_tree.heading("Tanggal", text="Order Date")
# riw_tree.heading("Tipe", text="Tipe Transaksi")
# riw_tree.heading("Pembeli", text="Pembeli")
# riw_tree.heading("Metode", text="Metode")
# riw_tree.heading("Fitur", text="Fitur")
#
# for col in ("ID", "Tanggal", "Tipe", "Pembeli", "Metode", "Fitur"):
#     riw_tree.column(col, anchor="center")
#
#
# # for col in ("ID", "Pembeli", "Tanggal", "Total"):
# #     riw_tree.column(col, anchor="center")  # Jangan pakai heading() lagi di sini
# riw_tree.pack(fill="both", expand=True)

# def on_treeview_double_click(event):
#     selected_item = riw_tree.selection()
#     if selected_item:
#         trx_id = selected_item[0]
#         tampilkan_detail_di_frame4(trx_id)
#
# riw_tree.bind("<Double-1>", on_treeview_double_click)
...
# === DETAIL TRANSAKSI SEBELAH KANAN ===
# riw_detail_frame = tk.Frame(riw_main, bg="white", bd=1, relief="solid")
# riw_detail_frame.pack(side="left", fill="both", expand=True)
#
# # Scrollable canvas
# detail_canvas = tk.Canvas(riw_detail_frame, bg="white")
# scroll_y = ttk.Scrollbar(riw_detail_frame, orient="vertical", command=detail_canvas.yview)
# scroll_frame = tk.Frame(detail_canvas, bg="white")
#
# scroll_frame.bind("<Configure>", lambda e: detail_canvas.configure(scrollregion=detail_canvas.bbox("all")))
# detail_canvas.create_window((0, 0), window=scroll_frame, anchor="nw")
# detail_canvas.configure(yscrollcommand=scroll_y.set)
#
# detail_canvas.pack(side="left", fill="both", expand=True)
# scroll_y.pack(side="right", fill="y")

# Simpan gambar agar tidak kehapus
riw_image_refs = {}
...
# def load_transaksi_image_async(url, label, nama):
#     def worker():
#         try:
#             from urllib.request import urlopen
#             response = urlopen(url)
#             img_data = Image.open(BytesIO(response.read()))
#             img_data.thumbnail((60, 60), Image.Resampling.LANCZOS)
#             img_tk = ImageTk.PhotoImage(img_data)
#
#             # Simpan & update di main thread
#             label.after(0, lambda: update_transaksi_image(label, img_tk, nama))
#         except Exception as e:
#             print(f"❌ Gagal load gambar transaksi {nama}: {e}")
#             label.after(0, lambda: label.config(text="[Gagal Load]"))
#
#     threading.Thread(target=worker).start()
#
# def update_transaksi_image(label, img, nama):
#     riw_image_refs[nama] = img
#     label.config(image=img, text="")
#     label.image = img
#
# def tampilkan_detail_transaksi(event):
#     selected = riw_tree.focus()
#     if not selected:
#         return
#     riw_image_refs.clear()
#     for widget in scroll_frame.winfo_children():
#         widget.destroy()
#
#     trx_data = db.reference("transaksi").child(selected).get()
#     if not trx_data or "barang" not in trx_data:
#         return
#
#     for item in trx_data["barang"]:
#         card = tk.Frame(scroll_frame, bg="#f9f9f9", bd=1, relief="solid", padx=10, pady=10)
#         card.pack(pady=5, padx=10, fill="x")
#
#         try:
#             items = db.reference("inventaris").order_by_child("nama").equal_to(item["nama"]).get()
#             gambar_url = ""
#             for _, val in items.items():
#                 gambar_url = val.get("gambar", "")
#                 break
#
#             if gambar_url:
#                 img_label = tk.Label(card, text="Loading...", bg="#f9f9f9")
#                 img_label.pack(side="left", padx=10)
#                 load_transaksi_image_async(gambar_url, img_label, item["nama"])
#             else:
#                 tk.Label(card, text="[No Image]", bg="#f9f9f9").pack(side="left", padx=10)
#
#         except Exception as e:
#             tk.Label(card, text="[No Image]", bg="#f9f9f9").pack(side="left", padx=10)
#
...
#         tk.Label(info, text=item["nama"], font=("Segoe UI", 12, "bold"), bg="#f9f9f9").pack(anchor="w")
#         tk.Label(info, text=f"Jumlah: {item['jumlah']}", bg="#f9f9f9").pack(anchor="w")
#         tk.Label(info, text=f"Harga: Rp {item['harga']:,}", bg="#f9f9f9").pack(anchor="w")
#
# riw_tree.bind("<<TreeviewSelect>>", tampilkan_detail_transaksi)
# riw.bind("<Visibility>", lambda e: load_riwayat())
...
#     ref = db.reference("inventaris")
#     data = ref.get()
#     barang_cb['values'] = [item['nama'] for item in data.values()] if data else []
#     edit_cb['values'] = [item['nama'] for item in data.values()] if data else []
def update_barang_cb():
...
# === Saat Pilih Barang, Tampilkan Otomatis ===
...
#
#         for kategori_data in data_inventaris.values():
#             for val in kategori_data.values():
#                 if val.get("nama") == nama:
#                     new_stok.delete(0, tk.END)
#                     new_harga.delete(0, tk.END)
#                     new_stok.insert(0, str(val.get("stok", 0)))
#                     new_harga.insert(0, str(val.get("harga", 0)))
#
#                     # Tampilkan gambar
#                     img_url = val.get("gambar", "")
#                     if img_url:
#                         from urllib.request import urlopen
...
#                     return
#     except Exception as e:
#         print(f"Gagal tampilkan data: {e}")
...
# def update_kategori_cb():
#     daftar_kategori = ambil_daftar_kategori()
#     kategori_cb['values'] = daftar_kategori
...
# kategori_cb = ttk.Combobox(kategori_frame, textvariable=kategori_terpilih, width=30, state="readonly")
# kategori_cb.pack(side="left", padx=5)
...
#
#         kotak = tk.Frame(kategori_inner_frame2, bg="white", bd=1, relief="solid", cursor="hand2")
#         kotak.pack(side="left", padx=10, pady=10)
#
#         label_img = tk.Label(kotak, text="[Loading]", bg="white")
#         label_img.pack()
#
#         # Ambil gambar dari URL
#         # def load_img(gambar_url, label, idx):
#         #     try:
#         #         img_data = Image.open(requests.get(gambar_url, stream=True).raw).resize((60, 60))
#         #         img_tk = ImageTk.PhotoImage(img_data)
#         #         label.config(image=img_tk, text="")
#         #         label.image = img_tk
#         #         kategori_images2.append(img_tk)
#         #     except:
#         #         label.config(text="[Gagal]")
#         def load_img(gambar_url, label, idx):
#             try:
#                 if not gambar_url:
...
#                 img_data = Image.open(response.raw).convert("RGBA").resize((60, 60))
#                 img_tk = ImageTk.PhotoImage(img_data)
#
#                 label.config(image=img_tk, text="")
#                 label.image = img_tk  # PENTING: agar gambar tidak hilang
#                 kategori_images2.append(img_tk)  # PENTING: simpan global
#
#             except Exception as e:
#                 print(f"[Gagal memuat gambar] URL: {gambar_url} | Error: {e}")
#                 label.config(image="", text="📦\n(no image)", compound="top", font=("Arial", 10))
#                 label.image = None  # kosongkan agar tidak error pas delete
#
#         if gambar_url:
#             threading.Thread(target=load_img, args=(gambar_url, label_img, i)).start()
#
#         tk.Label(kotak, text=nama, bg="white", font=("Segoe UI", 10)).pack(pady=(5, 0))
#
#         # Bind klik kategori
#         def on_click(nm=nama):
#             return lambda e: kategori_dipilih_dari_kotak2(nm)
#
#         kotak.bind("<Button-1>", on_click())
...
# kategori_cb.bind("<<ComboboxSelected>>", on_kategori_selected)
...
# === GUI Setup ===
canvas = Canvas(home2, width=1100, height=800, bg="white", bd=0, highlightthickness=0)
...
# tipe_transaksi_cb2 = ttk.Combobox(canvas, textvariable=transaksi_var2, values=["Dine-in", "Takeaway", "GoFood", "ShopeeFood", "GrabFood"])
# tipe_transaksi_cb2.place(x=280, y=485)  # atur posisi sesuai layout
...
# scroll_frame44 = Frame(canvas, bg="white")  # taruh langsung ke dalam canvas utama
# canvas.create_window((580, 200), window=scroll_frame44, anchor="nw", width=500, height=240)
#
# canvas44 = Canvas(scroll_frame44, width=480, height=240)
# canvas44.pack(side="left", fill="both", expand=True)
#
# scrollbar44 = Scrollbar(scroll_frame44, orient="vertical", command=canvas44.yview)
# scrollbar44.pack(side="right", fill="y")
#
# canvas44.configure(yscrollcommand=scrollbar44.set)
#
# inner_frame44 = Frame(canvas44, bg="yellow")
# canvas44.create_window((0, 0), window=inner_frame44, anchor="nw")
#
# inner_frame44.bind(
#     "<Configure>",
#     lambda e: canvas44.configure(scrollregion=canvas44.bbox("all"))
# )
#
# Label(inner_frame44, text="TEST MUNCUL", bg="red").pack(padx=10, pady=10)
...
# button = Button(home2, image=btn_img, bd=0, highlightthickness=0, relief="flat", command=simpan_transaksi_multi2)
# button.place(x=394, y=629, width=136, height=47)
...
# def update_quantity(nama, delta):
#     if nama in last_detected_items:
#         last_detected_items[nama]["quantity"] += delta
#         if last_detected_items[nama]["quantity"] < 1:
#             last_detected_items[nama]["quantity"] = 1  # Minimal 1 item
#     update_frame()  # refresh semua tampilan dan total

# def update_quantity(nama, delta):
#     if nama in keranjang_final:
#         current_qty = keranjang_final[nama]["quantity"]
#         new_qty = current_qty + delta
#
#         if new_qty <= 0:
#             keranjang_final.pop(nama, None)
#         else:
#             harga = keranjang_final[nama]["harga"]
#             keranjang_final[nama]["quantity"] = new_qty
#             keranjang_final[nama]["total"] = new_qty * harga

# def update_quantity(name, delta):
#     if name not in last_detected_items:
#         return  # Tidak bisa update item yang tidak terdeteksi
#
#     # Ambil qty lama (dari manual override atau AI)
#     current_qty = manual_quantity_overrides.get(name, last_detected_items[name]["quantity"])
#     new_qty = max(0, current_qty + delta)
#
#     if new_qty == last_detected_items[name]["quantity"]:
#         # Sama seperti hasil AI, jadi hapus override
#         manual_quantity_overrides.pop(name, None)
#     else:
#         # Simpan override
#         manual_quantity_overrides[name] = new_qty
#
#     update_keranjang_final()
...
def update_frame():
    ...
    #     qty = cnt_map[cls_idx]
    #
    #     # Gunakan quantity manual jika tersedia
    #     if name in manual_quantity_overrides:
    #         qty = manual_quantity_overrides[name]
    #
    #     harga = PRICES[name]
    #     total = qty * harga
    #     last_detected_items[name] = {
    #         "quantity": qty,
    #         "harga": harga,
    #         "total": total
    #     }
