...
# 🔥 Init Firebase
cred = credentials.Certificate("poskasir-develop-firebase-adminsdk-fbsvc-dbdeeb4379.json")
...
# root.configure(bg="#f0f2f5") # Removed as canvas_wrapper handles background

# Create a canvas to fill the entire root window for background control
canvas_wrapper = tk.Canvas(root, bg="white", highlightthickness=0)
canvas_wrapper.pack(fill="both", expand=True)

# Create a frame to hold all main content, sized to 1440x1080
content_frame = tk.Frame(canvas_wrapper, bg="white", width=1440, height=1080, bd=1, relief="solid")
content_frame.pack_propagate(False) # Prevent shrinking

# Center the content_frame within the canvas_wrapper
# We get actual screen dimensions after root is mapped
...
canvas_wrapper.create_window(x_center, y_center, window=content_frame, anchor="center")

# === Frame atas (header) ===
header = tk.Frame(content_frame, bg="white", height=60)
header.pack(side="top", fill="x")
header.pack_propagate(False)

# Logo di kiri (sesuaikan logo di sini)
original_image = Image.open("logodoang.png")  # pastikan filenya benar
...
# Sidebar (now packed relative to root as it's separate from main_area content)
sidebar = tk.Frame(content_frame, bg="#57708c", width=70)
sidebar.pack(side="left", fill="y")
sidebar.pack_propagate(False)

# Main area (berisi frame halaman)
main_area = tk.Frame(content_frame, bg="white", width=1440-70, height=1080) # 1440 total width - 70 sidebar width
main_area.pack(side="left", fill="both", expand=True)
main_area.pack_propagate(False) # Prevent shrinking
...
def show_frame(name):
    global is_updating_frame, cap

    # Stop update_frame and release camera if currently on Home2 (camera page)
    if is_updating_frame and name != "Home2":
        is_updating_frame = False
    ...
    if name in frames:
        ...
        # All content frames are children of main_area, so main_area is always packed
    ...
    update_header(user=current_user_name, halaman=name)
    if name == "Login":
        set_nav_state(False)
    else:
        set_nav_state(True)
    ...
    if name == "Home2":
        ...
        # Reinitialize camera when entering Home2
        if not cap or not cap.isOpened():
            cap = cv2.VideoCapture(0)
            print("[DEBUG] Camera re-initialized.")
        update_frame() # Start the continuous update
        update_header(user=current_user_name, halaman="Transaksi")
        tampilkan_kategori(home2) # Pass home2 as parent_frame for category buttons
    else:
        is_updating_frame = False
...
def load_inventaris():
    ...
    def show_kategori_items(nama_kategori):
        ...
        widths = [20, 40, 20, 10, 15, 25, 15]  # total = 145 "grid units", proporsional full HD
...
def tampilkan_daftar_barang(kategori):
    ...
    try:
        ...
        tk.Label(btn_tambah_baru, text="Add New Product", font=("Segoe UI", 10, "bold"), bg="white", fg="#556677").pack(pady=(5, 0))
...
MODEL_PATH = r"C:/AI/kue_pos/Hasil/yolo11n-seg(v1)/weights/best.pt"
USE_CAMERA = True
USE_STATIC_IMAGE = False
STATIC_IMAGE_PATH = r"C:/AI/kue_pos/assets/frame0/image_3.png"
USE_CUDA = True
IMG_SIZE = 896
CONFIDENCE = 0.5
IOU = 0.5
...
def simpan_transaksi_multi2():
    ...
    # ENTRY BAYAR
    bayar_var = tk.StringVar(value="")
    bayar_entry = ttk.Entry(left_frame, textvariable=bayar_var, font=("Segoe UI", 22), justify="right", state="readonly", width=22)
    ...
    for row in pad_buttons:
        ...
        for val in row:
            ...
            btn = tk.Button(
                row_frame,
                text=display,
                font=("Segoe UI", 14, "bold") if val.isdigit() and int(val) >= 1000 else ("Segoe UI", 13),
                width=6,
                height=2,
                command=lambda v=val: update_bayar(v)
            )
    ...
    def label_kanan(text, var=None, bold=False, pad=(10, 10)):
        tk.Label(right_frame, text=text, font=("Segoe UI", 13, "bold" if bold else "normal"),
                 bg="white", fg="#333").pack(anchor="w", padx=10, pady=(pad[0], 2))
        label = tk.Label(right_frame, textvariable=var, font=("Segoe UI", 14), bg="white") if var else \
            tk.Label(right_frame, text="", font=("Segoe UI", 14), bg="white")
        label.pack(anchor="w", padx=10, pady=(0, pad[1]))
        return label

    label_kanan("Total", pad=(0, 10))
    tk.Label(right_frame, text=f"idr {final_price:,}".replace(",", "."), font=("Segoe UI", 15, "bold"), bg="white").pack(anchor="w", padx=10)

    label_kanan("Paid", var=bayar_var, pad=(20, 10))

    tk.Frame(right_frame, height=2, bg="#ccc").pack(fill="x", pady=20)

    tk.Label(right_frame, text="Change", font=("Segoe UI", 13, "bold"), bg="white").pack(anchor="w", padx=10)
    kembalian_label = tk.Label(right_frame, text="", font=("Segoe UI", 14, "bold"), background="white", foreground="green")
...
# Generate random colors for other items, override specific ones
for kategori_data in data_inventaris.values():
    for item in kategori_data.values():
        nama = item.get("nama")
        ...
        if nama and nama not in CLASS_NAMES:
            ...
            DISCOUNTS[nama] = diskon if diskontotal is not None else 0
            # Set specific colors
            if nama == "Lemper":
                BOX_COLORS[nama] = "#00FF00"  # Green
            elif nama == "Kue Lapis":
                BOX_COLORS[nama] = "#8B4513"  # Brown
            elif nama == "Wajik":
                BOX_COLORS[nama] = "#FFFF00"  # Yellow
            elif nama == "Kue Mangkok":
                BOX_COLORS[nama] = "#FFC0CB"  # Pink
            elif nama == "Pastel":
                BOX_COLORS[nama] = "#808080"  # Grey
            else:
...
canvas.create_text(800 + 200, 55, anchor="nw", text="Cart", fill="#DBA576", font=("FamiljenGrotesk Bold", -24))  # geser kanan
...
current_order_id = 1
order_id = canvas.create_text(845, 135, anchor="nw", text=f"Order #[{current_order_id:04d}] --:--:--", fill="#000000", font=("microsoft sans serif", -24))
...
# Baris 4: Total akhir
canvas.create_text(790, footer_y + 90, anchor="nw", text="Total", fill="#000000", font=("microsoft sans serif", -22, "bold"))
...
def update_frame():
    ...
    # List to store label positions for collision detection
    label_positions = []

    for mask, box, cls_idx, conf in zip(masks, boxes, classes, confs):
        name = CLASS_NAMES[cls_idx]
        label = f"{name} {conf:.2f}"
        hexcol = BOX_COLORS[name][1:]  # Use BOX_COLORS for bounding box
        ...
        # Initial label position (above bounding box)
        label_x = x1 + 3
        label_y = y1 - 5
        ...
        for prev_x, prev_y, prev_w, prev_h in label_positions:
            # Simple AABB collision detection for labels
            if not (label_x + text_w < prev_x or
                    label_x > prev_x + prev_w or
                    label_y + text_h < prev_y or
                    label_y > prev_y + prev_h):
                overlap_found = True
                break
        
        if overlap_found:
            # Move label below bounding box if overlap
            label_y = y2 + text_h + 5
            # Ensure the label doesn't go off the bottom of the frame
            if label_y + text_h > disp.shape[0]:
                label_y = disp.shape[0] - 5 # Place at bottom if it would go off
        
        label_positions.append((label_x, label_y - text_h, text_w, text_h)) # Store actual bounding box of the text for next overlap check

        cv2.rectangle(disp, (label_x, label_y - text_h - 10), (label_x + text_w + 6, label_y), bgr, -1)
        cv2.putText(disp, label, (label_x, label_y - 5), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), thickness,
                    cv2.LINE_AA)
